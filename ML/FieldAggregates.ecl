...
/* //Unsuccessful attempt to avoid endless right-node propagation in KDTrees, however the code could be useful in other situation.
EXPORT LowHiHalfPos:=TABLE(SimpleRanked,{number; UNSIGNED lpos:=IF(COUNT(GROUP)=1, 1,MAX(GROUP,pos)/2); rpos:=IF(COUNT(GROUP)=1, 1, MAX(GROUP,pos) DIV 2 + MAX(GROUP,pos)%2 +1)},number,FEW);
EXPORT HiLowHalfs:=JOIN(Simpleranked,LowHiHalfPos,LEFT.number=RIGHT.number AND LEFT.pos = RIGHT.lpos,TRANSFORM({RECORDOF(SimpleRanked) AND NOT [id, pos]},SELF:=LEFT;),LOOKUP);
EXPORT LowHiHalfs:=JOIN(Simpleranked,LowHiHalfPos,LEFT.number=RIGHT.number AND LEFT.pos = RIGHT.rpos,TRANSFORM({RECORDOF(SimpleRanked) AND NOT [id, pos]},SELF:=LEFT;),LOOKUP);
*/
