...
EXPORT Trees := MODULE
  ...
  EXPORT cNode := RECORD
    NodeID;
    Types.NumericField;      // Continuous Instance Data
  ...
  END;
    ...
    NodeID;
    Types.t_FieldNumber number; // The column used to split
    Types.t_Discrete value; // The value for the column in question
  ...
  EXPORT SplitC := RECORD		// data structure for splitting results
    NodeID;
    Types.t_FieldNumber number; // The column used to split
    Types.t_FieldReal value; // The cutpoint value for the column in question
    t_node new_node_id; // The new node that value goes to
...
// which will be used by different decision tree learning algorithms (e.g. ID.3 Quilan)
  EXPORT PartitionGiniImpurityBased(DATASET(NodeInstDiscrete) nodes, t_level p_level, REAL Purity=1.0) := FUNCTION
		...
		nc0 := PROJECT(node_cand,TRANSFORM(NodeInstDiscrete,SELF.value := LEFT.new_nodeid,SELF.depend := LEFT.value,SELF.level := p_level, SELF.support:=LEFT.TCnt,SELF := LEFT,SELF := []));
		...
		J := JOIN(this_set,mapp,LEFT.id=RIGHT.id,TRANSFORM(NodeInstDiscrete,SELF.node_id:=RIGHT.nodeid,SELF.level:=LEFT.level+1,SELF := LEFT),LOCAL);
...
*/
  EXPORT Decision(DATASET(ML.Types.DiscreteField) ind,DATASET(Types.DiscreteField) dep,t_level Depth=10,REAL Purity=1.0) := MODULE
	...
	Split(DATASET(NodeInstDiscrete) nodes, t_level p_level) := FUNCTION
	  ...
	  node_base := MAX(aggc,node_id); // Start allocating new node-ids from the highest previous
		// Allocate the new node-ids
		node_cand := PROJECT(node_cand0,TRANSFORM({node_cand0, t_node new_nodeid},SELF.new_nodeid := node_base+COUNTER, SELF := LEFT));
		// Construct a fake NodeInstDiscrete to pass out splitting information
		nc0 := PROJECT(node_cand,TRANSFORM(NodeInstDiscrete,SELF.value := LEFT.new_nodeid,SELF.depend := LEFT.value,SELF.level := p_level,SELF := LEFT,SELF := []));
		// Construct a list of record-ids to (new) node-ids (by joining to the real data)
		r1 := RECORD
		  ML.Types.t_Recordid id;
			t_node nodeid;
...
// Returns a dataset with branch nodes and final nodes
  ...
  // It will be after C45PruneTree upgrade
  EXPORT SplitInstances(DATASET(SplitF) mod, DATASET(Types.DiscreteField) Indep) := FUNCTION
			...
			RETURN dedup1;
	END;
  // New function to travel instances to a final leaf node. Faster and more accurate than previous SplitInstances
  // It handles new instances' values not present in the model using weight
  EXPORT SplitInstancesD(DATASET(SplitF) mod, DATASET(Types.DiscreteField) Indep) := FUNCTION
    inst_wnode:= RECORD(Types.DiscreteField)
      NodeID;
      REAL8 weight;
    END;
    depth:=MAX(mod, level);
    ...
    test1:= JOIN(ind0, root, LEFT.number=RIGHT.number, TRANSFORM(inst_wnode, SELF.id:=LEFT.id, SELF.value:=LEFT.value, SELF.weight:=1.0, SELF:= RIGHT), LOOKUP);
    wNodeDiscrete := RECORD
      NodeID;
      Types.t_FieldNumber number; // The column used to split
      Types.t_Discrete value; // The value for the column in question
      t_node new_node_id; // The new node that value goes to
      Types.t_FieldReal weight;
    END;
    accmod:=TABLE(mod, {node_id, number, tot:= SUM(GROUP, support)},node_id, number);
    wNodes:=JOIN(mod, accmod, LEFT.node_id=RIGHT.node_id AND LEFT.number=RIGHT.number, TRANSFORM(wNodeDiscrete, SELF.weight:=LEFT.support/RIGHT.tot, SELF:=LEFT));
    loop_body(DATASET(inst_wnode) inst_nodes, t_level p_level) := FUNCTION
      instances:= JOIN(inst_nodes, ind0, LEFT.id=RIGHT.id AND LEFT.number=RIGHT.number, TRANSFORM(inst_wnode, SELF.value:=RIGHT.value, SELF:= LEFT), LOCAL);
      ...
      miss_val:=JOIN(join0(new_node_id=0), nodesN, LEFT.node_id=RIGHT.node_id, TRANSFORM(inst_wnode, SELF.weight:= LEFT.weight*RIGHT.weight, SELF.level:=LEFT.level+1, SELF.node_id:=RIGHT.new_node_id, SELF:=LEFT), LOOKUP, MANY);
      match_val:=PROJECT(join0(new_node_id>0), TRANSFORM(inst_wnode, SELF.node_id:=LEFT.new_node_id, SELF.level:=LEFT.level+1, SELF:=LEFT), LOCAL);
      all_val:= miss_val + match_val;
      RETURN JOIN(all_val, wNodes, LEFT.node_id=RIGHT.node_id, TRANSFORM(inst_wnode, SELF.number:= RIGHT.number, SELF.value:= RIGHT.value, SELF:= LEFT), LOOKUP);
    END;
