...
//Methods to handle Continuous Data with Decision Trees
  EXPORT BinaryPartitionC	(DATASET(cNode) nodes, t_level p_level, t_Count minNumObj=2) := FUNCTION
    ...
    node_dep := TABLE(root_sorted(number=1), {node_id, depend, high_fork, cnt:= COUNT(GROUP)}, node_id, depend, high_fork, MERGE);
    node_dep_tot := TABLE(node_dep, {node_id, tot:= SUM(GROUP, cnt)}, node_id, MERGE);
    tdp := RECORD
      node_dep;
      REAL4 prop; // Proportion based only on dependent value
      REAL4 plogp:= 0;
    ...
    node_dep_p:= JOIN(node_dep, node_dep_tot, LEFT.node_id = RIGHT.node_id, 
        TRANSFORM(tdp, SELF.prop:= LEFT.cnt/RIGHT.tot, SELF.plogp:= P_LOG_P(LEFT.cnt/RIGHT.tot), SELF:=LEFT));     
    node_entropy := TABLE(node_dep_p, {node_id, info:= SUM(GROUP, plogp)}, node_id, MERGE); // Information Entropy of nodes before split
    ...
    pass_thru_pure:= JOIN(node_dep, pure_node, LEFT.node_id = RIGHT.node_id, 
          TRANSFORM(cNode, SELF.id:= 0, SELF.number:= 0, SELF.value:= 0, SELF.level:= p_level, SELF:=LEFT), LOOKUP);
    // Calculating cut points
    // Compact to unique node-attribute values 
    root_impure:= JOIN(root_sorted, pure_node, LEFT.node_id = RIGHT.node_id, LEFT ONLY, LOOKUP);
    root_acc   := SORT(TABLE(root_impure, {node_id, number, value, cnt:=COUNT(GROUP)}, node_id, number, value, MERGE), node_id, number, value);
    r_compact  := PROJECT(root_acc, TRANSFORM({root_acc, t_FieldReal lval:=0, t_FieldReal cut:=0.0, t_Count next:= 0, BOOLEAN touse:=FALSE}, SELF:= LEFT));
    // Calculate cutpoint as middle point between two consecutive unique values belonging to the same node-attribute
    RECORDOF(r_compact) tx1(RECORDOF(r_compact) l, RECORDOF(r_compact) r):= TRANSFORM
      ...
      SELF:= r;
    END;
    root_cuts:= ITERATE(r_compact, tx1(LEFT, RIGHT))(touse = TRUE); // Filter garbage cutpoints generated by ITERATE
